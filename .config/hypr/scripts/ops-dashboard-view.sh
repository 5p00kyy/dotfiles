...
while true; do
  ...
  mem_line=$(free -h | awk '/Mem:/ {printf "%-8s / %-8s (%s)", $3, $2, int(($3+0)/($2+0)*100) "%" }' 2>/dev/null || true)
  ...
  printf '  updates      %s\n\n' "$updates"
  printf '  actions      [ q ] quit    [ d ] display menu    [ a ] accent toggle    [ l ] lock\n\n'
  printf '  top cpu\n'
